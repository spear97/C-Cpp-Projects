...
void Simulator::AddMember(int index, double x, double y, bool _adv) 
{
  ...
  int i = (index < 0) ? agents.size() : index;
...
void Simulator::Update() 
{
  ...
  if (Del_Index >= 0) agents.erase(agents.begin() + Del_Index);
}

void Simulator::SendControl(string control) 
{
  if(controlAgentIndex>=0 && controlAgentIndex<agents.size()) agents[controlAgentIndex].SetControl(control);
}

void Simulator::ToggleControlledAgent() 
{
  if(controlAgentIndex>=0 && controlAgentIndex<agents.size()) agents[controlAgentIndex].ToggleControlled();
}
void Simulator::ToggleControlledAdversary() 
{
  if(controlAgentIndex>=0 && controlAgentIndex<agents.size()) agents[controlAgentIndex].ToggleAdversary();
}

Vector3d Simulator::GetPointToFollow() 
{
  if(controlAgentIndex>=0 && controlAgentIndex<agents.size()) return agents[controlAgentIndex].GetPos();

  else return agents[0].GetPos();
} 